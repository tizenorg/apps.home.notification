...
EXPORT_API int notification_setting_refresh_setting_table()
{
	...
	pkgmgr_ret = pkgmgrinfo_pkginfo_privilege_filter_foreach(NOTIFICATION_PRIVILEGE, foreach_package_info_callback, db);
	if (pkgmgr_ret != PACKAGE_MANAGER_ERROR_NONE) {
		NOTIFICATION_ERR("pkgmgrinfo_pkginfo_privilege_filter_foreach failed [%d]", pkgmgr_ret);
		err = NOTIFICATION_ERROR_FROM_DB;
		goto out;
	}
